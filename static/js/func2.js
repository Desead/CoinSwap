...
function rates(side = 'test') {
    ...
    let input_value = +input_source.value
    let temp = 0;
    let prev = undefined;
    ...
    for (let i of swap_options[left + '_' + right]) {
        ...
        if (input_source.id === 'sum_from') {
            ...
            if ((+i.pay_from_min <= +input_value) && (+input_value <= +i.pay_from_max)) {
                temp = ((input_value * (+i.rate_to)) / (+i.rate_from) / 100 * (100 - (+i.fee)) - (+i.fee_fix)) / 100 * (100 - (+ps_fee_to[right].fee)) - (+ps_fee_to[right].fee_fix)
...
function clearFee() {
    let temp = document.querySelector('#fee_client')
    // console.log(temp)
    if (temp) {
        temp.value = ''
        rates('left')
